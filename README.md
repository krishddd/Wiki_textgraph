...
🟢 **v5.5.0 — schema-guided LLM extraction (ontology contract).** You can now pin a schema for the opt-in LLM relation pass: `textgraph build … --extract-schema ontology.json` takes an ontology of `entity_types` / `predicates` / typed `relations`, **constrains the prompt** to that vocabulary, and **drops any off-ontology triple** the model emits — so a schema raises precision instead of trusting the model. It's a field on `Config`, so it folds into the config hash and the LLM prompt-cache key (change the schema → re-extract under the new contract). Load it from JSON (dependency-free), or derive it from **Pydantic v2 models** with the optional `[schema]` extra. The pass stays opt-in and `GENERATED`-quarantined; a build with no schema is unchanged.

🟢 **v5.4.0 — clickable-to-region citation view.** Clicking a PDF citation in the Ask dock now shows **where on the page** it is: the source panel draws a **to-scale schematic of the cited page** with the bounding box highlighted, next to the cited text. It renders purely client-side from the citation's own `page` + `bbox` + `page_size` (page dims from the MediaBox, pure pypdf) — **no rasterizer, no extra dependency** — so it works offline and even when the raw bytes can't be shown (a PDF's binary differs from the extracted text it's cited by). A true page raster stays a future opt-in ([render] backend). Additive and byte-stable as ever: text-only corpora and every pre-5.4 graph are unchanged.
