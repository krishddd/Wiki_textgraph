...
## Post-v1.0 — v1.1 UI (in progress)

- [x] Deterministic force-directed layout (`l7_analytics/layout.py`), hash-seeded + fixed
      iterations, coords baked onto entity nodes; `graph.json` stays byte-identical
...
- [x] Shared canvas renderer (`console/renderer.py`): the live console and the offline
      `graph.html` render identically via a `TG` adapter (API vs embedded data +
      client-side path/search). Retires the concentric-ring SVG stub.
- [x] Retrieval-quality debt: second-stage reranker (`rerank.py`) interleaves answer
      entities with evidence passages — recall@5 0.60→0.70, MRR 0.29→0.80; cross-encoder
      behind `[rerank]`. Coverage gate widened from `core/` to whole-package (88% floor).
...
- [x] **UI / `textgraph console`**: dependency-free local web app over the QueryEngine —
      all 8 typed tools in the browser, cited, self-contained page (no CDN, G2), pure
